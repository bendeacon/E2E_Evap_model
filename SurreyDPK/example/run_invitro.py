    ...
    for i in range(Nsteps-1):
        ...
        if b_inf_source is True:
            ...
            if b_vary_vehicle is True:
                 mass[i, :] = _skin.compMass_comps()
                 m_v[i,0] = _skin.comps[0].getMass_OutEvap()
                 mass_evap = np.insert(mass, 0, m_v[i,0])
        ...
        if np.fabs((flux_vh_sc - flux_sc_ve) / flux_vh_sc) < 1e-5:
            break
        # elif i == Nsteps-1 :
...
#         total_mass_list.append(sum(_skin.compMass_comps()+m_v[i,0])/mass[0,0])
    ...
    Full_mass = m_v[:,0] + mass[:, 0] +mass[:, 1]+mass[:, 2]+mass[:, 3]+mass[:, 4] 
    ...
    df = pd.DataFrame({'time': t_list, 'flux_vh_sc': flux_vh_sc_list, 'flux_sc_ve': flux_sc_ve_list, \
                       ...
                       'evap':m_v[:,0]/Full_mass, 'mass0': mass[:,0]/Full_mass, 'mass1': mass[:,1]/Full_mass, \
                       'mass2': mass[:,2]/Full_mass, 'mass3': mass[:,3]/Full_mass, 'mass4': mass[:,4]/Full_mass,\
