...
def compPerm(fn_conf, chem=None, sc_Kw_paras=None, sc_D_paras=None, disp=1) :
    ...
    for i in range(Nsteps-1):
        ...
        flux_sc_down_list.append(flux_sc_down)
        ...
        sc_conc = sum(_skin.getComp(1,0).getMeshConc())/len(_skin.getComp(1,0).getMeshConc())
        sc_conc_list.append(sc_conc)
        ve_conc = sum(_skin.getComp(2,0).getMeshConc())/len(_skin.getComp(2,0).getMeshConc())
        ve_conc_list.append(ve_conc)
        de_conc = sum(_skin.getComp(3,0).getMeshConc())/len(_skin.getComp(3,0).getMeshConc())
        de_conc_list.append(de_conc)
        print( 'Vehicle conc =', vehicle_conc)
        # Simulate
    ...
    df = pd.DataFrame({'time':t_list, 'flux_vh_sc':flux_vh_sc_list, 'flux_sc_down': flux_sc_down_list, \
                       'vehicle_conc':vehicle_conc_list, 'sc_conc':sc_conc_list, 've_conc':ve_conc_list,\
                       'de_conc':de_conc_list})

    return flux_vh_sc / _conf.init_conc_vh, df
...
compPerm("C:\\Users\\dz0004\\E2E_Unilever_project\\source_code\\SurreyDPK\\example"+'\\config\\Caffeine_CE_copy.cfg')
