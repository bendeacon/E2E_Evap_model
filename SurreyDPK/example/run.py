...
_conf = config.Config(zz+'Caffeine_CE_copy.cfg')
...
for i in range(Nsteps):
    ...
    print('Time = ', t_range[i], 'Flux vh_sc= ', '{:.3e}'.format(_skin.compFlux([0,0], 3)[0]), \
       'Flux sc_down=', '{:.3e}'.format(_skin.compFlux([1,0], 3)[0]), 'total_mass=', sum(_skin.compMass_comps())/mass[0,0] )
